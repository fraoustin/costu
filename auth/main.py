...
@login_required
def currentuser():
    return render_template('user.html', user=current_user, backurl=None, locations=ParamApp.getValue('location').split(';'))
...
def view_user(id):
    try:
        ...
        if user is not None:
            return render_template('user.html', user=user, backurl="/users", locations=ParamApp.getValue('location').split(';'))
        else:
...
@checkAdmin()
def users():
    return render_template("users.html", users=User.all(sortby=User.name), backurl=None, locations=ParamApp.getValue('location').split(';'), authorizations=GroupOfAuthorization.all(sortby=GroupOfAuthorization.name))
...
class Auth(Blueprint):
    ...
    def init_db(self):    
        if len(User.query.all()) == 0:
            u = User(email='admin@localhost', name="Admin", password="admin", isadmin=True, token=str(uuid.uuid4()), apikey=str(uuid.uuid4()))
            db.session.add(u)
            db.session.commit()
